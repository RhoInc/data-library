(function (factory) {
    ...
    function makeMainDataList() {
      ...
      d3.json('./src/data-folders.json', function (data) {
        d3.select('.data-group').selectAll('div.data-grouping').data(data).enter().append('div').classed('data-grouping', true).each(function (d) {
          var container = d3.select(this);
          container.classed(d.subtype, true);
          var header = container.append('h2').classed('header', true);
          var link = header.append('a').attr('href', d.relPath).text(d.header.replace(' Datasets', ''));
          /* Expandable Details */

          if (!(['sdtm', 'adam'].indexOf(d.subtype) > -1 && d.folder !== d.subtype)) {
            var details = container.append('div').classed('details', true);
            var detailsHeader = details.append('div').classed('details-header', true).html('Details').on('click', function () {
              var detailsHeader = d3.select(this);
              var detailsToggle = detailsHeader.select('.details-toggle');
              var detailsContent = d3.select(this.parentNode).select('.details-content');
            ...
            var detailsToggle = detailsHeader.append('span').classed('details-toggle', true).text('+');
            var detailsContent = details.append('div').classed('details-content hidden', true);
          ...
          var filesHeader = files.append('div').attr('class', 'files-header').text(" ".concat(d.nDataFiles, " datasets")).on('click', function () {
          ...
          var filesToggle = filesHeader.append('span').classed('files-toggle toggle', true).text('+');
          ...
          files_lis.append('a').attr('title', function (d) {
            return 'Copy URL for ' + d.fileName + ' to clipboard';
          }).html("<i class='fa fa-clipboard'></i>").on('click', function (d) {
